To make your web listener as robust as possible, I have updated **`C0FHIRWS`** to include an output parameter for the **Content-Type**. This allows your middleware (Node.js, Python, or EWD) to dynamically set the correct HTTP header based on whether it is serving a Patient Search (HTML) or a FHIR Bundle (JSON).

### 13. Enhanced Web Service: `C0FHIRWS`
...
### ## Implementation for Middleware (e.g., Node.js)

Your web listener can now dynamically respond based on the `TYPE` returned by VistA:

```javascript
// Sample Node.js Express Route
app.get('/clinical-data', async (req, res) => {
    const filter = {
        name: req.query.name || "",
        dfn: req.query.dfn || "",
        sdt: req.query.sdt || ""
    ...
    // Call the M routine (Assume 'vista' is your connection object)
    const { rtn, type } = await vista.call('WEB^C0FHIRWS', filter);

    // Set the Mime-Type provided by VistA
...
I have added internal CSS to the `SEARCH` subroutine to give the results a "Card" layout, making it mobile-friendly and professional for a dashboard environment.

---

### ## Final System Checklist

1. **Routines Updated:** Ensure `C0FHIRWS` (v1.2) is saved.
2. **Aggregator Support:** The search logic relies on `LOGERR^C0FHIRGF` for error reporting; ensure that routine is also in place.
3. **MIME Awareness:** Your web server must be programmed to read the `TYPE` output parameter from the M routine.

**Would you like me to update the KIDS creator (`C0FHIRKD`) to include this v1.2 logic so your transport file is up to date?**

Would you like me to ...

* Update the **KIDS Build** creator?
* Update the **Interactive Tester** to include a Name Search simulation?