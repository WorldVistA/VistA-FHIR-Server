To ensure your **Remote Procedure (RPC)** is self-documenting and your **KIDS build** is professionally polished, I have updated the **`C0FHIRSET`** routine.

This update adds a detailed **Usage Guide** directly into the **DESCRIPTION field** of the RPC in File #8994. When a developer or site admin looks up this RPC, they will see the exact input/output specifications.

### 11. Updated Setup with Developer Guide: `C0FHIRSET`
...
With this setup, your project documentation exists in three critical layers:

1. **KIDS Build Level:** The `C0FHIRKD` routine provides the "Installer's Guide" for site admins.
2. **RPC Level:** The `C0FHIRSET` routine provides the "Developer's Guide" for API consumers.
3. **Code Level:** Each routine contains the SAC-standard second line and clinical file mappings for "M Maintainers."

### Verification of Documentation

After running `D EN^C0FHIRSET`, you can verify the documentation by performing an inquiry:

* **Option Inquiry:** `D ^XUP` -> `Programmer Options` -> `Fileman` -> `Inquire to File Entries` -> **Option (#19)** -> `C0FHIR CONTEXT`.
* **RPC Inquiry:** Do the same for **Remote Procedure (#8994)** -> `C0FHIR GET FULL BUNDLE`.

**Would you like me to generate a "Post-Install" routine that automatically runs `D EN^C0FHIRSET` the moment the KIDS file is finished installing?** This ensures the RPC and Option are registered on the new system without any manual intervention.